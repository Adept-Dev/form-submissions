...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . '/assets/app/employees/picture/' . $_POST['fname'] . $_POST['lname'] . '/';
$target_file = $target_dir . basename($_FILES['pic']['name']);
$uploadOk = 1;
$imgFileType = pathinfo($target_file, PATHINFO_EXTENSION);
$location = '/assets/app/employees/picture/' . $_POST['fname'] . $_POST['lname'] . '/' . basename($_FILES['pic']['name']);


// Check to see if dir already exist if not make one
if (!file_exists($_SERVER['DOCUMENT_ROOT'] . '/assets/app/employees/picture/' . $_POST['fname'] . $_POST['lname'])) {
    mkdir($_SERVER['DOCUMENT_ROOT'] . '/assets/app/employees/picture/' . $_POST['fname'] . $_POST['lname']);
} else {
